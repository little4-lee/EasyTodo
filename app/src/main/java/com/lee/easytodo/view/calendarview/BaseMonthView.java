...
public abstract class BaseMonthView extends BaseView {
     ...
     * @return
     */
    protected Calendar getCalendarByLocation(float x, float y) {
        if (mItemWidth == 0 || mItemHeight == 0) {
            return null;
        }
        int indexX = (int) (x - mDelegate.getCalendarPadding()) / mItemWidth;
        if (indexX >= 7) {
            indexX = 6;
        }
        int indexY = (int) y / mItemHeight;
        int position = indexY * 7 + indexX;// 选择项
        if (position >= 0 && position < mItems.size())
            return mItems.get(position);
        return null;
    ...
    final int getIndexOfCalendar(Calendar calendar) {
