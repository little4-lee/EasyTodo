...
public abstract class BaseView extends View implements View.OnClickListener, View.OnLongClickListener {
    ...
    public boolean onDragEvent(DragEvent event) {
        if (mDelegate.mDragActionInteractor == null) return false;
        else {
            switch (event.getAction()) {
                case DragEvent.ACTION_DRAG_LOCATION:
                    if (mDelegate.mDragActionInteractor.checkLocationUpdate(event.getX(), event.getY())) {
                        onLocationChanged(event.getX(), event.getY());
                    }
                    break;

                case DragEvent.ACTION_DROP:
                    onDrop(event.getX(), event.getY(), event.getClipData());
                    break;
                ...
                case DragEvent.ACTION_DRAG_EXITED:
                    onDragOut(event.getX(), event.getY());
                    break;
                default:
                    //do nothing
                    break;
            }
            return mDelegate.mDragActionInteractor.onDragEvent(event);
        }
